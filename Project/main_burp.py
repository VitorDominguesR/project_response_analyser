...
path_dirs = 'C:/Users/SWAT/Documents/Analises/1055/PTManager/'
#print os.walk(os.path.dirname(os.path.abspath(__file__))).next()[1]
vulns_dirs = os.walk(os.path.dirname(path_dirs)).next()[1]
print vulns_dirs
try:
    vulns_dirs.remove('.idea')
except:
    pass
attack_results = []
for dir in vulns_dirs:
    ...
    if path_file_request == "":
        path_file_request = path_dirs+dir+'/request_'+dir
        path_file_response = path_dirs+ dir + '/response_'+dir

    try:
        vuln_config_file = ET.parse(path_dirs+ dir+'/vuln.config').getroot()
